...
import org.zgg.gmall.realtime.bean.{OrderDetail, OrderInfo, OrderWide}
...
object DwdOrderApp {
  def main(args: Array[String]): Unit = {
    //1. 准备环境
    val sparconf: SparkConf = new SparkConf().setAppName("dwd_order_app").setMaster("local[2]")
    ...
    // OrderInfo(176,5,1001,38,77.0,0.0,0.0,69.0,8.0,0.0,2022-03-20 02:37:35,null,2022-03-20 02:22:35,null,null,null,null,null,null,null,0,null)
//    orderInfoDStream.print(100)
    ...
    // OrderDetail(402,169,28,129.0,1,索芙特i-Softto 口红不掉色唇膏保湿滋润 璀璨金钻哑光唇膏 Z03女王红 性感冷艳 璀璨金钻哑光唇膏 ,2022-03-20 02:22:35,99.0,0.0,30.0)
//    orderDetailDStream.print(100)
    ...
    // OrderInfo(196,11,1001,62,36195.0,500.0,0.0,36685.0,10.0,0.0,2022-03-21 02:47:54,null,2022-03-21 02:32:54,null,2022-03-21,02,江西,360000,CN-JX,CN-36,33,M)
//    orderInfoDimDStream.print(100)
    ...
    val orderDetailKVDStream: DStream[(Long, OrderDetail)]  =
      orderDetailDStream.map(orderDetail => (orderDetail.id, orderDetail))
        ...
        for ((key,(orderInfoOp,orderDetailOp)) <- orderJoinIter){
          if (orderInfoOp.isDefined){
            ...
            if (orderDetails != null && orderDetails.size()> 0){
              ...
              for (orderDetailsJson <- orderDetails.asScala){
                val orderDetail: OrderDetail = JSON.parseObject(orderDetailsJson, classOf[OrderDetail])
                val orderWide:OrderWide = new OrderWide(orderInfo, orderDetail)
                orderWides.append(orderWide)
              }
            }
