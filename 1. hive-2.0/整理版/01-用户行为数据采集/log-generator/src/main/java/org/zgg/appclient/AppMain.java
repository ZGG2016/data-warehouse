...
public class AppMain {
    ...
    public static void main(String[] args) {
        ...
        // 参数二：循环遍历次数
        int loop_len = args.length > 1 ? Integer.parseInt(args[1]) : 10000;
    ...
    private static void generateLog(Long delay, int loop_len) {
        ...
        for (int i = 0; i < loop_len; i++) {
            ...
            switch (flag) {
                ...
                case (1):
                    // 把每个事件封装成JSONObject，添加到JSONArray里，再将所有事件封装到JSONObject，再通过fastjson转成字符串
                    JSONObject json = new JSONObject();

                    json.put("ap", "app");
                    json.put("cm", generateComFields()); // JSONObject
